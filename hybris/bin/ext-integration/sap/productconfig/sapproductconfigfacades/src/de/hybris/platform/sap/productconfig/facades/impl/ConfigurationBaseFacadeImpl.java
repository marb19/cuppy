...
public class ConfigurationBaseFacadeImpl
{
	 ...
	 *
	 */
	private static final String LOG_TEST_MODE_MSG = "Running with local hybrisNameMap, instead of using session service. This mode is only for testing pupose and does not work in production mode with concurrent acces. Check injection of sessionService!";
	...
	private Map<String, HybrisCsticAndValueNames> localNameMap;
	...
	protected List<UiGroupData> getGroupsFromInstance(final InstanceModel instance,
			final Map<String, HybrisCsticAndValueNames> nameMap, final List<UiGroupData> csticGroupsFlat, int level)
	{
		level++;
		...
		{
			final UiGroupData uiGroup = createUiGroup(subInstance, nameMap, csticGroupsFlat, level);
	 ...
	 */
	protected UiGroupData createUiGroup(final InstanceModel instance, final Map<String, HybrisCsticAndValueNames> nameMap,
	...
	protected Map<String, HybrisCsticAndValueNames> getCachedNameMap()
	{
		Map<String, HybrisCsticAndValueNames> nameMap;

		if (sessionService != null)
		{
			nameMap = sessionService.getAttribute(HybrisCsticAndValueNames.class.getName());
			if (nameMap == null)
			{
	...
	protected void putCachedNameMap(final Map<String, HybrisCsticAndValueNames> nameMap)
	{
		...
		if (sessionService != null)
		{
			sessionService.setAttribute(HybrisCsticAndValueNames.class.getName(), nameMap);
		}
		else
		{
			LOG.warn(LOG_TEST_MODE_MSG);
