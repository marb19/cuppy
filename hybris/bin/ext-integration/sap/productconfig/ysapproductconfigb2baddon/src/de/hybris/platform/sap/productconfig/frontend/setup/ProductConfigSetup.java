...
public class ProductConfigSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		...
		if (isSOMActive(context)){			
			//take content without cart related statements
			Config.setParameter(impexPatternKey, "resources/impex/powertools/projectdataProduct*.impex");
		}
		else{
			//take all content
	...
	private boolean isSOMActive(final SystemSetupContext context)
	{
		final Class<OrderEntryData> orderEntryClass = OrderEntryData.class;
		final Class<?>[] parameterTypes = null;
		boolean result = true;
		try
		{
			orderEntryClass.getMethod(Sapproductconfigb2baddonConstants.CONFIGURABLE_SOM_DTO_METHOD, parameterTypes);
		}
		...
		logInfo(context,"Synchronous Order Management is active: " + result);
