...
public class ConfigDataMergeProcessorImpl implements ConfigDataMergeProcessor
{
	...
	public void mergeConfigurationData(final ConfigurationData source, final ConfigurationData target)
	{
		...
		if (CPQActionType.VALUE_CHANGED.equals(target.getCpqAction()))
		{
			if (source.isSingleLevel())
			{
				populateChangedValue(target);
			}

			target.setFocusId(null);
			target.setCpqAction(null);
		}
	...
	private void populateChangedValue(final ConfigurationData target)
	{
	...
	private Map<String, Object> prepareSourceConfiguration(final ConfigurationData source)
	{
	...
	private void processGroup(final UiGroupData group, final Map<String, Object> sourceConfigMap)
	{
	...
	private boolean updateGroup(final UiGroupData group, final Map<String, Object> sourceConfigMap)
	{
	...
	private void removeOutdatedConflictGroups(final List<UiGroupData> groups, final List<Integer> outdatedConflictGroupIndices)
	{
	...
	private void updateCstics(final UiGroupData group, final Map<String, Object> sourceConfigMap)
	{
		// update cstics
		final List<CsticData> cstics = group.getCstics();
		if (cstics != null && !cstics.isEmpty())
	...
	private boolean isChangedCsticWithAssignedValidationType(final CsticData cstic)
	{
