...
public class ConflictCheckerImpl implements ConflictChecker
{
	...
	private boolean isEmptyOrNull(final List<?> list)
	{
		final boolean isEmptyOrNull = list == null || list.isEmpty();
	...
	private boolean groupIsNotEmpty(final UiGroupData uiGroup)
	{
		final List<CsticData> cstics = uiGroup.getCstics();
		final List<UiGroupData> subGroups = uiGroup.getSubGroups();
		final boolean isNotEmpty = !isEmptyOrNull(cstics) || !isEmptyOrNull(subGroups);
		return isNotEmpty;
	}
	...
	protected boolean checkAllCsticCopmplete(final UiGroupData uiGroup)
	{
		...
		if (cstics != null && cstics.size() > 0)
		{
			if (uiGroup.isVisited())
			{
				...
				{
					if (cstic.isRequired() && checkEmpty(cstic))
					{
						isComplete = false;
						break;
					}
	...
	private boolean hasNoErrorOrWarningOrConflict(final UiGroupData uiGroup)
	{
		final GroupStatusType groupStatus = uiGroup.getGroupStatus();
		final boolean noErrorOrWarningOrConflict = !GroupStatusType.ERROR.equals(groupStatus)
				&& !GroupStatusType.WARNING.equals(groupStatus) && !GroupStatusType.CONFLICT.equals(groupStatus);
		return noErrorOrWarningOrConflict;
	}
}
