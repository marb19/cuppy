...
<div class="${cssConf:valueStyleClass(cstic)} ${valueStyle}">
	<c:choose>
		<c:when test="${cstic.maxlength > 0}">
			<form:input id="${csticKey}.inputNum" class="form-control" type="text" path="${pathPrefix}value" maxlength="${cstic.maxlength}"
				placeholder="${cstic.placeholder}" />
		</c:when>
		<c:otherwise>
			<form:input id="${csticKey}.inputNum" type="text" path="${pathPrefix}value" placeholder="${cstic.placeholder}" />
		</c:otherwise>
	</c:choose>
</div>
