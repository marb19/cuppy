...
<div id="${csticKey}.checkBoxList" class="${cssConf:valueStyleClass(cstic)} ${valueStyle}">
	<c:forEach var="value" items="${cstic.domainvalues}" varStatus="status">
		...
		<div class="checkbox">
			<form:checkbox id="${value.key}.checkBox" class="capitalize cpq-csticValueSelect ${cssValueClass}"
				disabled="${value.readonly}" path="${pathPrefix}domainvalues[${status.index}].selected" value="${value.selected}" />
			<label id="${value.key}.label" for="${value.key}.checkBox" class="cpq-csticValueLabel ${cssLabelClass}">${value.langdepname}</label>
			<c:if test="${value.deltaPrice.formattedValue ne '-'}">
				...
				<div id="${value.key}.deltaPrice" title="${formattedPrice}"
