...
public class ErrorResolverTest
{
	...
	public void setup()
	{
		...
		errors = new BeanPropertyBindingResult(configData, Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE);
		...
		ObjectError validationError = new FieldError(Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, getPath(0, 0),
		...
		errors.addError(conflictError);
		validationError = new FieldError(Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, getPath(0, 1),
				VALIDATION_ERROR_MESSAGE);
		errors.addError(validationError);
		...
		errors.addError(conflictError);
		validationError = new FieldError(Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, getPath(1, 0),
				VALIDATION_ERROR_MESSAGE);
		errors.addError(validationError);
		...
		{ "", "" }, MAND_FIELD_ERROR_MESSAGE);
		errors.addError(mandatoryFieldError);
		validationError = new FieldError(Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, "groups[0].subGroups[0].cstics[0]",
				VALIDATION_ERROR_MESSAGE);
		errors.addError(validationError);
		...
		final Object[] args =
		{ "arg0", "arg1", "arg2" };
		final ObjectError validationErrorWithArg = new FieldError(Sapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, getPath(0, 2),
	...
	@Test
	public void testGetConflictErrorsWithoutDuplicates()
	{

		final ConflictError conflictError = new ConflictError(null, getPath(0, 3), null, null, CONFLICT_ERROR_MESSAGE);
		//add conflict twice
		errors.addError(conflictError);
		errors.addError(conflictError);
		final List<ErrorMessage> conflictErrors = ErrorResolver.getConflictErrorsWithoutDuplicates(errors);
		assertNotNull(conflictErrors);
		assertEquals("Have to be three conflicts", 3, conflictErrors.size());
		assertEquals(CONFLICT_ERROR_MESSAGE, conflictErrors.get(0).getMessage());
		assertEquals(getPath(0, 3), conflictErrors.get(0).getPath());
	}
	...
	{
		List<ErrorMessage> conflictErrors = ErrorResolver.getWarningsForGroup(errors, "groups[1].");

		assertNotNull(conflictErrors);
		assertEquals("Has to be two errors", 2, conflictErrors.size());
		assertEquals(CONFLICT_ERROR_MESSAGE, conflictErrors.get(0).getMessage());

		conflictErrors = ErrorResolver.getWarningsForGroup(errors, "groups[0].");

		assertNotNull(conflictErrors);
		assertEquals("Has to be four errors", 4, conflictErrors.size());
		assertEquals(CONFLICT_ERROR_MESSAGE, conflictErrors.get(0).getMessage());
		assertEquals(MAND_FIELD_ERROR_MESSAGE, conflictErrors.get(1).getMessage());
	}

	@Test
	public void testWarningsOnSubGroupLevel()
	{
		final List<ErrorMessage> conflictErrors = ErrorResolver.getWarningsForGroup(errors, "groups[0].subGroups[0].");

		assertNotNull(conflictErrors);
		assertEquals("Have to be four errors", 1, conflictErrors.size());
		assertEquals(MAND_FIELD_ERROR_MESSAGE, conflictErrors.get(0).getMessage());
	}

	@Test
	public void testErrorsOnGroupLevel()
	{
		List<ErrorMessage> validationErrors = ErrorResolver.getErrorsForGroup(errors, "groups[1].");

		assertNotNull(validationErrors);
		assertEquals("Has to be one error", 1, validationErrors.size());
		assertEquals(VALIDATION_ERROR_MESSAGE, validationErrors.get(0).getMessage());

		validationErrors = ErrorResolver.getErrorsForGroup(errors, "groups[0].");
	...
	{
		final List<ErrorMessage> validationErrors = ErrorResolver.getErrorsForGroup(errors, "groups[0].subGroups[0].");

		assertNotNull(validationErrors);
		assertEquals("Have to be one errors", 1, validationErrors.size());
		assertEquals(VALIDATION_ERROR_MESSAGE, validationErrors.get(0).getMessage());
	}
}
