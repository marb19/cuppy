...
public class SolvableConflictAdapterImpl implements SolvableConflictAdapter
{
		...
		try
		{
			...
			if (solvableConflicts != null)
			{
				final List<SolvableConflictModel> solvableConflictModelList = Arrays.asList(solvableConflicts) //
						.stream() //
						.map(conflict -> createSolvableConflictModel(conflict)) //
	...
	protected SolvableConflictModel createSolvableConflictModel(final IConflictingAssumptionsContainer solvableConflict)
	{
		...
		if (assumptions != null)
		{
			...
			if (conflictingAssumptionsList.size() > 0)
			{
	...
	static ConflictingAssumptionModel createConflictingAssumptionsModel(final IAssumptions assumption)
	{
	...
	private String getAssumptionId(final String csticName, final SolvableConflictModel solvableConflict)
	{
