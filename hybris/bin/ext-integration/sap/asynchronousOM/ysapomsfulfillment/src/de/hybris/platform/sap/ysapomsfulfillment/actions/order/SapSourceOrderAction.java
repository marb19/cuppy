...
public class SapSourceOrderAction extends AbstractProceduralAction<OrderProcessModel> {
	...
	public void executeAction(final OrderProcessModel process) throws RetryLaterException, Exception {
		...
		if (results != null) {
			...
			startSapConsignmentSubProcess(consignments, process);
	...
	/**
	 * * Create and start an SAP consignment process for every consignment
	 * @param consignments
	 * @param process
	 */
	protected void startSapConsignmentSubProcess(final Collection<ConsignmentModel> consignments, final OrderProcessModel process)
	{
		for (final ConsignmentModel consignment : consignments)
		{
			final SapConsignmentProcessModel subProcess = getBusinessProcessService().<SapConsignmentProcessModel>createProcess(
					process.getOrder().getCode() + "_sap_cons_" +
					consignment.getCode() + "_ordermanagement", YsapomsfulfillmentConstants.CONSIGNMENT_SUBPROCESS_NAME);
