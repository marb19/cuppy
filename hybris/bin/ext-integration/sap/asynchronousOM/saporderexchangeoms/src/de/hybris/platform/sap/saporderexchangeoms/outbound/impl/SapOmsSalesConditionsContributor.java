...
public class SapOmsSalesConditionsContributor extends DefaultSalesConditionsContributor
{
	...
	public List<Map<String, Object>> createRows(final OrderModel order)
	{
		...
		return super.syncPricingInactive(entries) ? createRowsHybrisPricing(order) : createRowsSyncPricing(order);
	}
	 ...
	 */
	private List<Map<String, Object>> createRowsSyncPricing(final OrderModel order)
	{
	...
	protected List<Map<String, Object>> createRowsHybrisPricing(final OrderModel order)
	{
		...
		for (final AbstractOrderEntryModel entry : order.getEntries())
		{
			for (final ConsignmentEntryModel consignmentEntry : entry.getConsignmentEntries())
			{
				createGrossPriceRow(consignmentEntry, result);
				createTaxRows(consignmentEntry, result);
				createDiscountRows(consignmentEntry, result);
			}
		}
	...
	protected void createGrossPriceRow(final ConsignmentEntryModel consignmentEntry, final List<Map<String, Object>> result)
	{
		final Map<String, Object> row = new HashMap<>();
		final AbstractOrderEntryModel entry = consignmentEntry.getOrderEntry();

		row.put(OrderCsvColumns.ORDER_ID, entry.getOrder().getCode());
		...
		row.put(SalesConditionCsvColumns.CONDITION_CURRENCY_ISO_CODE, entry.getOrder().getCurrency().getIsocode());
	...
	protected void createDiscountRows(final ConsignmentEntryModel consignmentEntry, final List<Map<String, Object>> result)
	{
		final AbstractOrderEntryModel entry = consignmentEntry.getOrderEntry();
	...
	protected void createTaxRows(final ConsignmentEntryModel consignmentEntry, final List<Map<String, Object>> result)
	{
		final AbstractOrderEntryModel entry = consignmentEntry.getOrderEntry();
