...
public class DefaultB2BCustomerAddressInterceptorTest
{
	...
	public void testExportDataIfPhoneIsModified() throws InterceptorException
	{
		...
		AddressModel shippingAddressModel = mock(AddressModel.class);
		given(b2bCustomerModel.getDefaultShipmentAddress()).willReturn(shippingAddressModel);
		given(ctx.isModified(b2bCustomerModel.getDefaultShipmentAddress(), shippingAddressModel.PHONE1)).willReturn(true);
		defaultCustomerAddressInterceptor.setB2bContactExportService(b2bContactExportService);
		...
		// then
		verify(b2bContactExportService, times(1)).prepareAndSend(b2bCustomerModel,shippingAddressModel,Boolean.TRUE,"en");
	}
