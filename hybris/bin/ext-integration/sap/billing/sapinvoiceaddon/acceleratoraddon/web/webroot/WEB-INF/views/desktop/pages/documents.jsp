...

{
	...
	"documentListerHeaderHtml" : "<spring:escapeBody javaScriptEscape="true">
						...
						<tr>
								...
								<th id="invoice"><spring:theme code="text.company.accountsummary.invoice.label" text="Invoice"/></th>
	...
	"documentListerHtml" :  "<spring:escapeBody javaScriptEscape="true">
					<tbody>
						...
						<c:forEach items="${searchPageData.results}" var="result">
							<spring:url value="/my-company/organization-management/invoicedocument/invoicedetails"
												var="invoiceDetailsURL">
								<spring:param name="invoiceCode" value="${result.documentNumber}"/>
								<spring:theme code="text.company.accountsummary.invoice.label" var="documentTypeName"/> 
							</spring:url>
							<tr>
								<td><spring:theme code="text.company.accountsummary.${result.status }.label"/></td>
								<td>${result.documentType.name }</td>
								<c:choose>
									<c:when test="${result.documentType.name eq fn:trim(documentTypeName)}">
										<td><a href="${invoiceDetailsURL}">${result.documentNumber}</a></td> 	
									</c:when>
									<c:otherwise>
								...
								<c:if test="${result.documentType.name eq fn:trim(documentTypeName)}">
								<td>
										<a href="#" id="invoicePDF" data-invoicenumber="${result.documentNumber}" data-errormsg="${defaultErrorMsg}" class="invoiceClass"><spring:theme code="download.view" text="Download"/></a>
								</td>
								</c:if>
								<td>&nbsp;</td>
							</tr>
