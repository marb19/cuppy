...
public class ApplicationPropertyResolverTest
{
	...
	public void noAppId()
	{
		...
		ApplicationPropertyResolver apr = new ApplicationPropertyResolver(yaasConfig);
		YaasApplicationModel model = mock(YaasApplicationModel.class);
	...
	{
		final YaasConfigurationService yaasConfig = mock(YaasConfigurationService.class);
		ApplicationPropertyResolver apr = new ApplicationPropertyResolver(yaasConfig, "appId");
		YaasApplicationModel model = mock(YaasApplicationModel.class);
		doReturn("clientId").when(model).getClientId();
		doReturn("clientSecret").when(model).getClientSecret();
