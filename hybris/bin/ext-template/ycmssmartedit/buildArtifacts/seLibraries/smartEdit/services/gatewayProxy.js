...
angular.module('gatewayProxyModule', ['gatewayFactoryModule'])
     ...
     */
    .factory('gatewayProxy', function($log, $q, hitch, gatewayFactory) {
        ...
        var turnToPromise = function(fnName, _service) {
            var originalMethod = _service[fnName];
            _service[fnName] = hitch(originalMethod, function() {
                return $q.when(this.apply(_service, arguments));
            });
        };
