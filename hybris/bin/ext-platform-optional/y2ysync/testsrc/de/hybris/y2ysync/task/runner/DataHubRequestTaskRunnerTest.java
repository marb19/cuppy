...
public class DataHubRequestTaskRunnerTest
{
	...
	public void shouldUseDataHubUrlFromY2YSyncJobIfPresent() throws Exception
	{
		...
		// then
		final Y2YSyncContext ctx = new Y2YSyncContext(FEED_NAME, POOL_NAME, TARGET_SYSTEM);
		verify(requestCreator).sendRequest(eq(TEST_EXECUTION_ID), eq(TEST_DATAHUB_URL), refEq(ctx));
	}
	...
	public void shouldUseDataHubUrlFromPropertiesIfY2YSyncJobHasntConfiguredItDirectly() throws Exception
	{
		...
		// then
		final Y2YSyncContext ctx = new Y2YSyncContext(FEED_NAME, POOL_NAME, TARGET_SYSTEM);
		verify(requestCreator).sendRequest(eq(TEST_EXECUTION_ID), eq(DEFAULT_DATAHUB_URL), refEq(ctx));
	}
