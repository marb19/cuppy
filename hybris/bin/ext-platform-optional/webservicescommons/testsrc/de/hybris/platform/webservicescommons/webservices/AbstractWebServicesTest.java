...
public abstract class AbstractWebServicesTest extends ServicelayerTest//NOPMD
{
	...
	{
		super();
		this.useSSL = Registry.getCurrentTenant().getConfig().getString(WS_REQUIRED_CHANNEL_PROPERTY, "https").equals("https") ? true
				: false;
		final int embeddedTomcatHttpPort = Registry.getCurrentTenant().getConfig()
				.getInt(EMBEDDEDSERVER_HTTP_PORT_CONFIG_KEY, 8001);
		final int embeddedTomcatHttpsPort = Registry.getCurrentTenant().getConfig()
				.getInt(EMBEDDEDSERVER_HTTPS_PORT_CONFIG_KEY, 8002);
		this.embeddedTomcatPort = useSSL ? embeddedTomcatHttpsPort : embeddedTomcatHttpPort;

		this.oauthEndpointPath = Registry.getCurrentTenant().getConfig()
				.getString(WS_TEST_OAUTH2_TOKEN_ENDPOINT_PATH_KEY, "/oauth/token");
		...
		this.oauthCustomerName = Registry.getCurrentTenant().getConfig()
				.getString(WS_TEST_OAUTH2_CUSTOMER_USERNAME_KEY, "testoauthcustomer");
		this.oauthCustomerPassword = Registry.getCurrentTenant().getConfig()
				.getString(WS_TEST_OAUTH2_CUSTOMER_PASSWORD_KEY, "1234");
	}
	...
	private static void restoreOriginalCacheSuffix()
	{
		...
		else
		{
			Registry.getCurrentTenant().getConfig()
					.setParameter(TenantAwareEhCacheManagerFactoryBean.CACHE_SUFFIX_PROPERTY, originalCacheSuffix);
	...
	protected WebTarget setupWebResource(final ExtensionInfo extensionInfo)
	{
		...
		return jerseyClient.target(UriBuilder.fromUri(fromUri).port(embeddedTomcatPort)
				.path(getWsVersionForExtensionInfo(extensionInfo)).build());
	}
	...
	protected EmbeddedServer createEmbeddedServer()
	{
		...
		embeddedServerCtx = getEmbeddedServerBuilder().needEmbeddedServer().withApplication(
				new EmbeddedExtension(getExtensionInfo()).withContext(wsVersion));
		...
		if (getAuthorizationExtensionInfo() != null)
		{
			...
			embeddedServerCtx = embeddedServerCtx.withApplication(new EmbeddedExtension(getAuthorizationExtensionInfo())
					.withContext(authExtWsVersion));
	...
	private void setCacheSuffix()
	{
		...
		Registry.getCurrentTenant().getConfig()
				.setParameter(TenantAwareEhCacheManagerFactoryBean.CACHE_SUFFIX_PROPERTY, "testCache");
