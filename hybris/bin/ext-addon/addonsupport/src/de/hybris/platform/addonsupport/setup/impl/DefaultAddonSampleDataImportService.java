...
public class DefaultAddonSampleDataImportService extends AbstractSystemSetup implements AddOnSampleDataImportService
{
	...
	private static final String IMPORT_STORES_URL = "/import/stores/";
	private static final String STORES_URL = "/stores/";
	private static final String CONTENT_CATALOGS_URL = "/contentCatalogs/";
	private static final String IMPORT_PRODUCT_CATALOGS_URL = "/import/productCatalogs/";
	...
	protected void importProductCatalog(final SystemSetupContext context, final String importDirectory, final String catalogName)
	{
		...
		// Load Units
		importImpexFile(context, "/" + importDirectory + IMPORT_PRODUCT_CATALOGS_URL + catalogName
		...
		importImpexFile(context,
				"/" + importDirectory + IMPORT_PRODUCT_CATALOGS_URL + catalogName + "ProductCatalog/products.impex", false);
		...
		importImpexFile(context, "/" + importDirectory + IMPORT_PRODUCT_CATALOGS_URL + catalogName + "ProductCatalog/users.impex",
				false);
	...
	protected void importContentCatalog(final SystemSetupContext context, final String importDirectory, final String catalogName)
	{
		...
		{
			importImpexFile(context, importRoot + CONTENT_CATALOGS_URL + catalogName + "ContentCatalog/cms-responsive-content.impex",
					false);
		}
	...
	protected void importStore(final SystemSetupContext context, final String extensionName, final String storeName)
	{
		...
		importImpexFile(context, "/" + extensionName + IMPORT_STORES_URL + storeName + "/store.impex", false);
		importImpexFile(context, "/" + extensionName + IMPORT_STORES_URL + storeName + "/site.impex", false);
			...
			final List<String> storeNames, final String productCatalog, final List<String> contentCatalogs, final boolean solrReindex)
	{
		...
		importImpexFile(context, "/" + importDirectory + IMPORT_PRODUCT_CATALOGS_URL + productCatalog
				+ "ProductCatalog/reviews.impex", false);

		processStoreNames(context, importDirectory, storeNames, productCatalog, solrReindex);
	}
	...
	protected void processStoreNames(final SystemSetupContext context, final String importDirectory,
								   final List<String> storeNames, final String productCatalog, 
								   final boolean solrReindex)
	{
		for (final String storeName : storeNames)
		{
			// Load promotions after synchronization is done
			importImpexFile(context, "/" + importDirectory + IMPORT_STORES_URL + storeName + "/promotions.impex", false);
