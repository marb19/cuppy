...
public class ChineseSummaryCheckoutStepController extends SummaryCheckoutStepController
{
	...
	@RequireHardLogIn
	public void doPayment(@PathVariable("orderCode") final String orderCode, final HttpServletRequest request,
			...
			CommerceCartModificationException, IOException
	{
		...
		response.sendRedirect(chinesePaymentService.getPaymentRequestUrl(orderCode));
	}
			...
			throws CMSItemNotFoundException, InvalidCartException, CommerceCartModificationException
	{
		...
		orderData = chineseCheckoutFacade.getOrderDetailsForCode(orderCode);
			...
			InvalidCartException, CommerceCartModificationException, IOException
	{
		...
		chinesePaymentService.handleAsynResponse(request, response);
	}
