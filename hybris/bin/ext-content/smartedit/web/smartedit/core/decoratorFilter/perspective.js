 ...
 *
 */
angular.module('PerspectiveModule', ['ApplicationManager', 'decoratorServiceModule', 'ngCookies'])

.constant('PerspectiveCookieVersion', "1.0")

.factory('Perspective', function() {
    var Perspective = function(name, decorators, loadFunc, system) {
        this.name = name;
        this.system = system === undefined ? false : system;
        this.dynamicDecoratorLoader = loadFunc;
        this.localDecorators = decorators;
        this.setDecorators = function(decorators) {
            var newDecorators = [];
            for (var pk in decorators) {
                newDecorators.push(decorators[pk]);
            }
            this.localDecorators = newDecorators;
        };
        this.getDecorators = function() {
            if (this.dynamicDecoratorLoader) {
                return this.dynamicDecoratorLoader();
            }
            return this.localDecorators;
        };
    };

    Perspective.prototype.clone = function() {
        var decoratorClone = [];
        var thisDecorators = this.getDecorators();
        for (var pk in thisDecorators) {
            decoratorClone.push(thisDecorators[pk]);
        }
        return new Perspective(this.name, decoratorClone, this.dynamicDecoratorLoader, this.system);
    };

    return Perspective;
})

.factory('systemPerspectives', function(Perspective, ApplicationManagerService) {
    var perspectiveAll = new Perspective(
        'perspectives.perspectivename.all', [],
        function() {
            return ApplicationManagerService.getAllDecorators();
        },
        true);
            ...
            if (perspectivesArr) {
                for (var index in perspectivesArr) {
                    serializedData.perspectives.push({
                        ...
                        decorators: perspectivesArr[index].getDecorators()
                    });
                }
            }
...
.factory('decoratorFilterService', function(decoratorService, perspectiveStorage, Perspective) {
    ...
    return {
        ...
        setDecoratorsForPerspective: function(perspectiveName, decorators) {
            for (var p in perspectives) {
                if (perspectives[p].name === perspectiveName) {
                    ...
                    perspectives[p].setDecorators(decorators);
        ...
        storePrecompiledComponent: function(componentId, componentType, content) {
            var key = componentId + componentType;
            precompiledComponents[key] = {
                id: componentId,
                type: componentType,
                html: content
            };
        ...
        getPrecompiledComponents: function() {
            //TODO - return a copy not reference
...
.controller('PerspectiveUIController', function($scope, $modalInstance, Perspective, decoratorFilterService, ApplicationManagerService) {
    ...
    $scope.perspectiveChanged = function() {
        ...
        updateDecoratorSet($scope.perspective.getDecorators());
    };
    ...
    $scope.save = function() {
        ...
        $scope.perspective.setDecorators(enabledDecoratorsForSelectedSet);
    };
