...
angular.module('gatewayFactoryModule', ['eventServiceModule'])
     ...
     */
    .service('gatewayFactory', function($rootScope, $q, hitch, getOrigin, systemEventService, customTimeout, $log, $window, $injector, WHITE_LISTED_STOREFRONTS_CONFIGURATION_KEY) {

        var PROMISE_ACKNOWLEDGEMENT_EVENT_ID = 'promiseAcknowledgement';
        var PROMISE_RETURN_EVENT_ID = 'promiseReturn';
        var SUCCESS = 'success';
        var FAILURE = 'failure';
        var TIMEOUT_TO_RETRY = 100;
        ...
        var MessageGateway = function MessageGateway(gatewayId) {
            ...
            this.publish = function(eventId, data, retries, pk) {
                ...
                var target = this._getTargetFrame();
                ...
                customTimeout(hitch(this, function(deferred, retries) {
                    if (!deferred.acknowledged && eventId !== PROMISE_RETURN_EVENT_ID && eventId !== PROMISE_ACKNOWLEDGEMENT_EVENT_ID) { //still pending
                        if (retries < 5) {
                            retries++;
                            $log.debug(document.location.href, "retrying", eventId);
                            this.publish(eventId, data, retries, pk).then(hitch(deferred, function(resolvedDataOfLastSubscriber) {
                                this.resolve(resolvedDataOfLastSubscriber);
                            }), hitch(deferred, function() {
                                this.reject();
                            }));
                        } else {
                            deferred.reject();
                ...
                }, deferred, retries), TIMEOUT_TO_RETRY);
