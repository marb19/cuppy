 ...
 *
 */
angular.module('experienceSelectorModule', ['eventServiceModule', 'genericEditorModule', 'sharedDataServiceModule', 'loadConfigModule', 'iframeClickDetectionServiceModule', 'restServiceFactoryModule', 'iFrameManagerModule', 'languageServiceModule', 'siteServiceModule', 'catalogServiceModule', 'genericEditorModule', 'resourceLocationsModule'])
    .directive('experienceSelector', function(systemEventService, languageService, catalogService, siteService, sharedDataService, loadConfigManagerService, iframeClickDetectionService, restServiceFactory, iFrameManager, GenericEditor, PREVIEW_RESOURCE_URI, copy, hitch, $log, $q, $filter, $document) {
        return {
            ...
            link: function($scope) {
                ...
                function adaptExperienceToScopeExperience(experience) {
                    selectedExperience.catalog = {
                        label: experience.catalogDescriptor.name + ' - ' + experience.catalogDescriptor.catalogVersion,
                            ...
                            catalogService.getCatalogsForSite(siteDescriptor.uid).then(function(catalogVersionDescriptors) {
                                catalogVersionDescriptors.forEach(function(catalogVersionDescriptor) {
                                    var candidateDropdownChoice = {
                                        label: catalogVersionDescriptor.name + ' - ' + catalogVersionDescriptor.catalogVersion,
                ...
                $document.on('click', function(event) {
                    if ($(event.target).parents('#option_0').length <= 0) {
