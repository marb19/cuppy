    ...
    describe('between frames', function() {
        ...
        it('WHEN I drag a component from the outer frame into the inner frame and move through slots, THEN I expect the right component position to be highlighted', function() {
            ...
            getElementPosition('#slot1 #component1').then(function(elementPosition) {
                ...
                dragToPosition(0, -5);
                expect(element.all(by.css('#slot1 > div > div')).first().getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Second position highlighted
                browser.sleep(500);
                dragToPosition(0, 90);
                expect(element.all(by.css('#slot1 > div > div')).get(1).getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Third position highlighted
                browser.sleep(500);
                dragToPosition(0, 70);
                expect(element.all(by.css('#slot1 > div > div')).get(2).getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Last position highlighted
                browser.sleep(500);
                dragToPosition(0, 70);
                expect(element.all(by.css('#slot1 > div > div')).last().getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);
            });
        });
