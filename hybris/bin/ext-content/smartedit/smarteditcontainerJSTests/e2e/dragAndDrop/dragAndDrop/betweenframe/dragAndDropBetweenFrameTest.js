    ...
    describe('between frames', function() {
        ...
        it("from parent frame, component0 will be dragged and dropped to slot1 in the iframe", function() {
            ...
            getElementPosition('#slot1 #component1').then(function(elementPosition) {
                ...
                dropDraggedElement().then(function() {
                    browser.switchToParent();
                    expect(element(by.binding('alert.message')).getText()).toBe('This item is dropped to slot:slot1 at index:1');
        ...
        it('WHEN I drag a component from the outer frame into the inner frame and move through slots, THEN I expect the right component position to be highlighted', function() {
            ...
            getElementPosition('#slot1 #component1').then(function(elementPosition) {
                ...
                dragToPosition(0, -5);
                expect(element.all(by.css('#slot1 > div.ng-scope > div')).first().getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Second position highlighted
                browser.sleep(500);
                dragToPosition(0, 90);
                expect(element.all(by.css('#slot1 > div.ng-scope > div')).get(1).getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Third position highlighted
                browser.sleep(500);
                dragToPosition(0, 70);
                expect(element.all(by.css('#slot1 > div.ng-scope > div')).get(2).getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);

                // Last position highlighted
                browser.sleep(500);
                dragToPosition(0, 70);
                expect(element.all(by.css('#slot1 > div.ng-scope > div')).last().getAttribute('class')).toContain(CSS_CLASSES.SORTABLE_PLACEHOLDER);
            });
        });
        ...
        it("from parent frame, component0 is not droppable into secondarySlot1", function() {
            ...
            element(by.css('#secondarySlot1 #secondaryComponent1')).getLocation().then(function(navDivLocation) {
                ...
                expect(element(by.css('#secondarySlot1 #component0')).isPresent()).toBe(false);
                expect(element(by.css('#secondarySlot2 #component0')).isPresent()).toBe(false);
            ...
            expect(element(by.css('#secondarySlot1 div[data-smartedit-component-type=componentType0]')).isPresent()).toBe(false);
            expect(element(by.css('#secondarySlot1 div[data-smartedit-component-type=componentType0]')).isPresent()).toBe(false);
