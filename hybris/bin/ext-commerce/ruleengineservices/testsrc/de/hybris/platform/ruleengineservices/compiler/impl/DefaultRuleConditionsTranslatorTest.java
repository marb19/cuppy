...
public class DefaultRuleConditionsTranslatorTest
{
	...
	public void setUp()
	{
		...
		ruleConditionsTranslator.setRuleConditionsRegistry(ruleConditionsRegistry);
	...
	public void translateConditions() throws Exception
	{
		...
		when(ruleConditionsRegistry.getAllConditionDefinitionsAsMap())
				.thenReturn(Collections.singletonMap(CONDITION_DEFINITION_ID, ruleConditionDefinition));
		when(applicationContext.getBean(CONDITION_TRANSLATOR_ID, RuleConditionTranslator.class))
				.thenReturn(ruleConditionTranslator);

		when(ruleConditionTranslator.translate(ruleCompilerContext, ruleCondition, ruleConditionDefinition))
				.thenReturn(ruleIrCondition);
	...
	public void conditionTranslatorNotFound() throws Exception
	{
		...
		when(ruleConditionsRegistry.getAllConditionDefinitionsAsMap())
				.thenReturn(Collections.singletonMap(CONDITION_DEFINITION_ID, ruleConditionDefinition));
		when(applicationContext.getBean(CONDITION_TRANSLATOR_ID, RuleConditionTranslator.class))
				.thenThrow(new NoSuchBeanDefinitionException(CONDITION_TRANSLATOR_ID));
