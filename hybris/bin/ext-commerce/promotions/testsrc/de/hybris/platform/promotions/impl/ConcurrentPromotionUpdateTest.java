...
public class ConcurrentPromotionUpdateTest extends ServicelayerTest
{
	...
	public void testConcurrentPromotionUpdate() throws Exception
	{
		...
		final TestThreadsHolder runners = new TestThreadsHolder(THREADS, new ConcurrentAddToCartRunner(cart,
				JaloSession.getCurrentSession(), prGroup3), true);
		runners.startAll();
		...
		final PromotionResultModel promoResult2 = (PromotionResultModel) promoResultIter.next();

		// this is just a potential promotion, which does not change the prices
		assertEquals("Product_Stepped_Multi_Buy", promoResult1.getPromotion().getCode());
