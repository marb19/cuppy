...
public class DefaultB2BApproverFacadeIntegrationTest extends AbstractCommerceOrgIntegrationTest
{
	...
	{
		final SearchPageData<UserData> searchPageData = defaultB2BApproverFacade.getPagedApproversForUnit(DEFAULT_PAGEABLE_DATA,
				DC_SALES_US);
	...
	public void testRemoveApproverFromUnit()
	{
		...
		SearchPageData<UserData> searchPageData = defaultB2BApproverFacade
	...
	{
		final SearchPageData<UserData> searchPageData = defaultB2BApproverFacade.getPagedApproversForCustomer(
				PAGEABLE_DATA_0_20_BY_UNIT_NAME, DC_S_DET);
		assertSearchPageData(4, searchPageData);

		final List<UserData> selectedApprovers = B2BCommercefacadesTestUtils.getSelectedUsers(searchPageData.getResults());
	...
	{
		SearchPageData<UserData> searchPageData = defaultB2BApproverFacade.getPagedApproversForCustomer(
				PAGEABLE_DATA_0_20_BY_UNIT_NAME, DC_S_DET);
		assertSearchPageData(4, searchPageData);

		List<UserData> selectedApprovers = B2BCommercefacadesTestUtils.getSelectedUsers(searchPageData.getResults());
		assertEquals(UNEXPECTED_NUMBER_OF_APPROVERS_SELECTED, 1, selectedApprovers.size());
