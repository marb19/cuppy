...
public class DefaultAssistedServiceFacade implements AssistedServiceFacade
{
	...
	public Map<String, Object> getAssistedServiceSessionAttributes()
	{
		...
		attributes.put("emulateByOrder", Config.getParameter(AssistedservicefacadesConstants.EMULATE_BY_ORDER_PROPERTY));

		final String errorMessage = getAsmSession().getFlashErrorMessage();
		if (errorMessage != null)
		{
			attributes.put("asm_message", errorMessage);
			attributes.put("asm_message_args", getAsmSession().getFlashErrorMessageArgs());
		}
		return attributes;
	}
			...
			throws AssistedServiceFacadeException
	{
		...
		try
		{
			UserModel customer = getCustomer(customerId);
			...
			if ((customer instanceof CustomerModel) && !isAssistedServiceAgent(customer))
			{
				...
				if (StringUtils.isNotBlank(cartId))
				{
					...
					if (cart == null)
					{
						final OrderModel order = gerOrderByCode(cartId, customer);
						if (order == null)
						{
				...
				{
					final OrderModel order = gerOrderByCode(orderId, customer);
					if (order == null)
					{
	...
	private void attachLatestSessionCart(final UserModel customer)
	{
	...
	private OrderModel gerOrderByCode(final String orderCode, final UserModel customer)
	{
		final StringBuffer buf = new StringBuffer();
	...
	private CartModel getLatestModifiedCart(final UserModel customer)
	{
		return CollectionUtils.isNotEmpty(customer.getCarts()) ? customer.getCarts().stream()
				.filter(cart -> isAbstractOrderMatchBaseSite(cart)).max(Comparator.comparing(CartModel::getModifiedtime)).get()
				: null;
	}
	 ...
	 */
	protected boolean isAbstractOrderMatchBaseSite(final AbstractOrderModel abstractOrderModel)
	{
