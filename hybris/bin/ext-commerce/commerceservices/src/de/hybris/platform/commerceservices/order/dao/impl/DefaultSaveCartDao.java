...
public class DefaultSaveCartDao extends DefaultCommerceCartDao implements SaveCartDao
{
	...
	protected final static String FIND_SAVED_CARTS_FOR_USER_AND_SITE = SELECTCLAUSE + "WHERE {" + CartModel.USER
			+ "} = ?user AND {" + CartModel.SITE + "} = ?site AND " + SAVED_CARTS_CLAUSE + " " +  ORDERBYCLAUSE;

	protected final static String FIND_SAVED_CARTS_FOR_USER = SELECTCLAUSE + "WHERE {" + CartModel.USER
			+ "} = ?user AND " + SAVED_CARTS_CLAUSE + " " +  ORDERBYCLAUSE;

	protected final static String FIND_EXPIRED_SAVED_CARTS_FOR_SITE = SELECTCLAUSE + "WHERE {" + CartModel.SITE
			+ "} = ?site AND " + SAVED_CARTS_CLAUSE + " AND {" + CartModel.EXPIRATIONTIME + "} <= ?currentDate "
			+ ORDERBYCLAUSE;
	...
	protected final static String FIND_SAVED_CARTS_FOR_SITE_AND_USER_WITH_STATUS = "SELECT {" + CartModel.PK + "} FROM {"
			...
			+ "} = ?site AND " + SAVED_CARTS_CLAUSE + " AND {OrderStatus.CODE} in (?orderStatus) " + ORDERBYCLAUSE;
	...
	protected final static String FIND_SAVED_CARTS_FOR_USER_WITH_STATUS = "SELECT {" + CartModel.PK + "} FROM {"
			...
			+ " AND {OrderStatus.CODE} in (?orderStatus) " + ORDERBYCLAUSE;
