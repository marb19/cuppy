...
public class ConcurrentPromotionEngineServiceTest extends PromotionEngineServiceBaseTest
{
	...
	public void testConcurrentPromotionUpdate() throws Exception
	{
		...
		final DroolsRuleModel rule = getRuleForFile("percentageDiscountCameraAccessories.drl",
		...
		final ProductModel product = productService.getProductForCode(
				catalogVersionService.getCatalogVersion("hwcatalog", "Online"), "HW1210-3411");
		...
		assertEquals("promotions-module-junit", rule.getKieBase().getKieModule().getName());

		final int THREADS = 50;

		final TestThreadsHolder runners = new TestThreadsHolder(THREADS, new ConcurrentUpdatePromotionsRunner(cart,
				JaloSession.getCurrentSession(), promoGroup1), true);
		runners.startAll();
	...
	{
		final RuleEngineActionResult result = commerceRuleEngineService.initialize(getTestRulesModule(Arrays.stream(rules).collect(
				Collectors.toSet())));
